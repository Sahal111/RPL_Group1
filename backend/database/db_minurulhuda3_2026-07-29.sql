...
# Dump of table guru_anaks
# ------------------------------------------------------------

DROP TABLE IF EXISTS `guru_anaks`;

CREATE TABLE `guru_anaks` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
  `guru_id` bigint unsigned NOT NULL COMMENT 'FK ke gurus.id',
  `nama` varchar(150) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Nama lengkap anak',
  `jenis_kelamin` enum('L','P') COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `tanggal_lahir` date DEFAULT NULL,
  `urutan` tinyint unsigned NOT NULL DEFAULT '1' COMMENT 'Anak ke-1, ke-2, dst',
...
CREATE TABLE `guru_kompetensi` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
  `guru_id` bigint unsigned NOT NULL COMMENT 'FK ke gurus.id',
  `jenis` enum('bahasa','it','bidang_keahlian','lainnya') COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Jenis kompetensi',
  `nama` varchar(150) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Nama kompetensi: Bahasa Inggris, Microsoft Excel, dll',
  `tingkat` enum('Dasar','Menengah','Mahir','Ahli') COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Tingkat kemampuan',
  `keterangan` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
...
DROP TABLE IF EXISTS `guru_kontak_darurat`;

CREATE TABLE `guru_kontak_darurat` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
  `guru_id` bigint unsigned NOT NULL COMMENT 'FK ke gurus.id',
  `nama` varchar(150) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Nama kontak darurat',
  `hubungan` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Hubungan: Istri, Suami, Orang Tua, Saudara, dll',
  `no_hp` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Nomor HP yang bisa dihubungi',
  `alamat` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `is_primary` tinyint(1) NOT NULL DEFAULT '1' COMMENT '1=Kontak utama',
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
...
# Dump of table guru_mutasi
# ------------------------------------------------------------

DROP TABLE IF EXISTS `guru_mutasi`;
...
# Dump of table guru_pkgs
# ------------------------------------------------------------

DROP TABLE IF EXISTS `guru_pkgs`;

CREATE TABLE `guru_pkgs` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
  `guru_id` bigint unsigned NOT NULL COMMENT 'FK ke gurus.id',
  `tahun_ajaran_id` bigint unsigned DEFAULT NULL COMMENT 'FK ke tahun_ajarans.id',
  `semester_id` bigint unsigned DEFAULT NULL COMMENT 'FK ke semesters.id',
  `nilai` decimal(5,2) DEFAULT NULL COMMENT 'Nilai PKG 0.00–100.00',
  `predikat` enum('Amat Baik','Baik','Cukup','Sedang','Kurang') COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `catatan` text COLLATE utf8mb4_unicode_ci COMMENT 'Catatan dari kepala sekolah',
  `dinilai_oleh` bigint unsigned DEFAULT NULL COMMENT 'FK ke users.id (kepala sekolah)',
  `tanggal_penilaian` date DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_guru_pkg_ta_sem` (`guru_id`,`tahun_ajaran_id`,`semester_id`),
  KEY `idx_gurupkg_guru_id` (`guru_id`),
  KEY `fk_gurupkg_ta` (`tahun_ajaran_id`),
  KEY `fk_gurupkg_sem` (`semester_id`),
  KEY `fk_gurupkg_penilai` (`dinilai_oleh`),
  CONSTRAINT `fk_gurupkg_guru` FOREIGN KEY (`guru_id`) REFERENCES `gurus` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_gurupkg_penilai` FOREIGN KEY (`dinilai_oleh`) REFERENCES `users` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_gurupkg_sem` FOREIGN KEY (`semester_id`) REFERENCES `semesters` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_gurupkg_ta` FOREIGN KEY (`tahun_ajaran_id`) REFERENCES `tahun_ajarans` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Penilaian Kinerja Guru (PKG) per semester';
...
CREATE TABLE `guru_rekenings` (
  ...
  `no_bpjs_kesehatan` varchar(30) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Nomor kartu BPJS Kesehatan 13 digit',
  `no_bpjs_ketenagakerjaan` varchar(30) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Nomor kartu BPJS Ketenagakerjaan',
...
CREATE TABLE `guru_sertifikasis` (
  ...
  `lptk` varchar(200) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Nama LPTK penyelenggara sertifikasi guru',
...
CREATE TABLE `gurus` (
  ...
  `dusun` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Nama dusun/kampung',
  ...
  `no_wa` varchar(20) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Nomor WhatsApp aktif (bisa beda dengan no_hp)',
  ...
  `kewarganegaraan` varchar(30) COLLATE utf8mb4_unicode_ci DEFAULT 'WNI' COMMENT 'WNI atau WNA',
  `status_hidup` enum('Aktif','Meninggal') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'Aktif' COMMENT 'Status hidup guru',
  ...
  `status_kepegawaian` enum('PNS','PPPK','GTY','GTT','Honorer','Lainnya') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Status kepegawaian: PNS|PPPK|GTY (Tetap Yayasan)|GTT (Tidak Tetap)|Honorer|Lainnya',
  `status_aktif` tinyint(1) NOT NULL DEFAULT '1' COMMENT '1=Guru masih aktif mengajar. 0=Pensiun/resign/mutasi keluar',
  `status_keaktifan` enum('Aktif','Cuti','Pensiun','Mutasi','Keluar') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'Aktif' COMMENT 'Status keaktifan guru saat ini',
  ...
  `no_sk_pengangkatan` varchar(80) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Nomor SK pengangkatan pertama',
  `tgl_sk_pengangkatan` date DEFAULT NULL COMMENT 'Tanggal SK pengangkatan',
  `instansi_pengangkat` varchar(150) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Instansi yang mengeluarkan SK: Kemenag, Yayasan, dll',
  `masa_kerja_tahun` tinyint unsigned DEFAULT NULL COMMENT 'Masa kerja dalam tahun (untuk display, diupdate berkala)',
